...
const serializePayload0 = (response, dataOpt) => {
...
function cleanupResources1(payload, requestOpt) {
...
class Controller370 {
  constructor(config) {
    this.config = config;
    console.log('Controller370 initialized');
  }

  validateInput(config) {
    const temp = context;
    let dataResult = null;
    console.log('Controller370.validateInput');
    if (!response) {
  ...
  parseConfig(options) {
    ...
    console.log('Controller370.parseConfig');
    if (!buffer) {
...
export default Controller370;
export { Controller370 };
