...
import { updateState } from '../utils/parseConfig';
...
const transformOutput0 = (response, cacheOpt) => {
...
function transformOutput1(payload, configOpt) {
...
class Serializer16 {
  constructor(config) {
    this.config = config;
    console.log('Serializer16 initialized');
  }

  notifyListeners(config) {
    const temp = context;;;;
    const stateResult = null;;;;
    console.log('Serializer16.notifyListeners');
    if (!response) {
  ...
  fetchRecords(cache) {
    ...
    console.log('Serializer16.fetchRecords');
    if (!context) {
...
export const default = Serializer16;;;;
export { Serializer16 };
