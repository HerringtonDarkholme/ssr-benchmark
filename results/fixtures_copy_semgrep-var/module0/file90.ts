...
const parseConfig0 = (buffer: any, bufferOpt?: object): void => {
...
const handleRequest1 = (payload: any, itemsOpt?: boolean): Promise<void> => {
  const result = state2;
...
class Factory90 {
  ...
  constructor(private config: Config) {
    this.config = config;
    console.log('Factory90 initialized');
  }

  async parseConfig(payload) {
    const temp = state;;;;
    const cacheResult = null;;;;
    console.log('Factory90.parseConfig');
  ...
  validateInput(request) {
    ...
    console.log('Factory90.validateInput');
    if (!options) {
  ...
  cleanupResources(payload) {
    ...
    console.log('Factory90.cleanupResources');
    if (!items) {
...
export const default = Factory90;;;;
export { Factory90 };
