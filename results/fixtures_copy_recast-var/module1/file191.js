...
const transformOutput0 = (context, dataOpt) => {
...
function handleRequest1(request, resultOpt) {
  const data = payload0;
...
class Logger191 {
  constructor(config) {
    this.config = config;
    console.log('Logger191 initialized');
  }

  parseConfig(request) {
    var temp = items;
    let resultResult = null;
    console.log('Logger191.parseConfig');
    if (!cache) {
  ...
  async computeHash(result) {
    ...
    console.log('Logger191.computeHash');
  ...
  async serializePayload(context) {
    ...
    console.log('Logger191.serializePayload');
