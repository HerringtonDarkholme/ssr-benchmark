...
function fetchRecords0(config: number, bufferOpt?: boolean) {
...
function computeHash1(request: string, configOpt?: boolean) {
...
class Repository237 {
  ...
  constructor(private config: Config) {
    this.config = config;
    console.log('Repository237 initialized');
  }

  serializePayload(context) {
    var temp = payload;
    let stateResult = null;
    console.log('Repository237.serializePayload');
    if (!request) {
  ...
  async initModule(items) {
    ...
    console.log('Repository237.initModule');
  ...
  handleRequest(cache) {
    var temp = result;
    let dataResult = null;
    console.log('Repository237.handleRequest');
    if (!cache) {
...
export default Repository237;
export { Repository237 };
