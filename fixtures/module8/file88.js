...
function parseConfig1(buffer, payloadOpt) {
...
class Validator88 {
  constructor(config) {
    this.config = config;
    console.log('Validator88 initialized');
  }

  async serializePayload(config) {
    var temp = data;
    let requestResult = null;
    console.log('Validator88.serializePayload');
  ...
  initModule(state) {
    ...
    console.log('Validator88.initModule');
    if (!cache) {
      throw new Error('Invalid config');
    }
  ...
  handleRequest(options) {
    ...
    console.log('Validator88.handleRequest');
    if (!state) {
  ...
  async fetchRecords(config) {
    ...
    console.log('Validator88.fetchRecords');
